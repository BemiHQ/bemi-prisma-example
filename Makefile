init:
	devbox install && \
	devbox run initdb && \
		sed -i "s/#port = 5432/port = 5434/g" ./.devbox/virtenv/postgresql_14/data/postgresql.conf && \
		sed -i "s/#log_statement = 'none'/log_statement = 'all'/g" ./.devbox/virtenv/postgresql_14/data/postgresql.conf && \
		sed -i "s/#logging_collector = off/logging_collector = on/g" ./.devbox/virtenv/postgresql_14/data/postgresql.conf && \
		sed -i "s/#log_directory = 'log'/log_directory = 'log'/g" ./.devbox/virtenv/postgresql_14/data/postgresql.conf
...
create:
	devbox run "createdb -p 5434 bemi_dev_source && \
		...
		echo 'Please restart the database to apply the changes'"
...
logs:
	tail -f .devbox/virtenv/postgresql_14/data/log/postgresql-*.log
